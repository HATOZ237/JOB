...
def main(ngen):
    ...
    # interval = (-3,7)
    func = [random(), random(), random(), random()]
    mu = func
    sigma = 0.5
    ...
    logbook.record(gen=g, fitness=best.fitness.values[0], loss=loss[round(abs(best[0] * 4)) % 4],
                   learning_rate=learning_rate[round(abs(best[1] * 3)) % 3], l1_ratio=abs(best[2] % 1),
                   alpha=10 ** (-4 * best[3]), score=score(best))
