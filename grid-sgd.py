...
param_Grid = {"learning_rate":["optimal", 'invscaling', "adaptive", 'constant'], "l1_ratio":np.linspace(0,1, num = 100), "alpha":np.logspace(-4,0,num = 100), "loss":["hinge", 'log', 'perceptron', "modified_huber","squared_hinge"]}
...
for i, (name, data) in enumerate(zip(names, datasets)):
    model =  SGDClassifier(n_jobs=-1, eta0=0.00001, max_iter = 10000)
...
pd.DataFrame(grid_dict).to_pickle("GRIDRESULTS-SGD")
