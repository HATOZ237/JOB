...
param_Grid = {"max_features":np.linspace(0.001,0.999, num=100), "max_samples":np.linspace(0.001,0.999, num=100), "n_estimators":np.arange(1, 100, 1)}
...
for i, (name, data) in enumerate(zip(names, datasets)):
    ...
    if(not(target_names[i] is None)):
        ...
        grid_t = GridSearchCV(model, param_grid=param_Grid, cv=3, n_jobs=-1, verbose = 2)
...
pd.DataFrame(results_grid).to_csv("GRIDSEARCH-RF")
pd.DataFrame(grid_dict).to_pickle("GRIDRESULTS-RF")
